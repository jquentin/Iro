...
public class ScaleTweenable : Tweenable {
	...
	public void TweenScaleFrom(Vector3 from, Vector3 to, float time, float delay = 0f, iTween.EaseType easeType = iTween.EaseType.easeOutExpo)
	{
	...
	{
		TweenScaleFrom(Vector3.one * from, Vector3.one * to, time, delay, easeType);
	}

	public void TweenScaleFrom(float from, Vector3 to, float time, float delay = 0f, iTween.EaseType easeType = iTween.EaseType.easeOutExpo)
	{
		TweenScaleFrom(Vector3.one * from, to, time, delay, easeType);
	}

	public void TweenScaleFrom(Vector3 from, float to, float time, float delay = 0f, iTween.EaseType easeType = iTween.EaseType.easeOutExpo)
	{
		TweenScaleFrom(from, Vector3.one * to, time, delay, easeType);
	}

	public void TweenScale(Vector3 to, float time, float delay = 0f, iTween.EaseType easeType = iTween.EaseType.easeOutExpo)
	{
		iTween.ScaleTo(gameObject, iTween.Hash(
			"scale", to,
			"time", time,
	...
	{
		TweenScale(Vector3.one * to, time, delay, easeType);
	}
