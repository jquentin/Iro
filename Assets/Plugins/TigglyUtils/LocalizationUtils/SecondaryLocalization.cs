...
//---------------------------------------------------------------------------------------------------------------------------------
// Secondary localization. For use in Safari
//---------------------------------------------------------------------------------------------------------------------------------

public static class SecondaryLocalization
{
	/// <summary>
	...
	static public Dictionary<string, string[]> dictionary
	{
		get
		{
			if (!localizationHasBeenSet) language = PlayerPrefs.GetString("SecondaryLanguage", "English");
			return mDictionary;
		}
		set
		{
	...
	static public string[] knownLanguages
	{
		get
		{
			if (!localizationHasBeenSet) LoadDictionary(PlayerPrefs.GetString("SecondaryLanguage", "English"));
			return mLanguages;
		}
	}
	...
	static public string language
	{
		get
		{
			if (string.IsNullOrEmpty(mLanguage))
			{
				...
				mLanguage = PlayerPrefs.GetString("SecondaryLanguage", lan != null ? lan[0] : "English");
	...
	static bool LoadAndSelect (string value)
	{
		...
		mDictionary.Clear();
		if (string.IsNullOrEmpty(value)) PlayerPrefs.DeleteKey("SecondaryLanguage");
		return false;
	}
		...
		if (mDictionary.TryGetValue("KEY", out keys))
		{
			for (int i = 0; i < keys.Length; ++i)
			{
				if (keys[i] == language)
				{
					...
					mLanguageIndex = i;
					mLanguage = language;
					PlayerPrefs.SetString("SecondaryLanguage", mLanguage);
	...
	{
//		return (isArabic ? ArabicSupport.ArabicFixer.Fix(text) : text);
		return text;
	}
	...
	static public string Get (string key)
	{
		...
		if (mLanguageIndex != -1 && mDictionary.TryGetValue(mobKey, out vals))
		{
			if (mLanguageIndex < vals.Length)
				return FixArabic(vals[mLanguageIndex]);
		}
		else if (mOldDictionary.TryGetValue(mobKey, out val)) return FixArabic(val);
		#endif
		...
		else if (mOldDictionary.TryGetValue(key, out val)) return FixArabic(val);
